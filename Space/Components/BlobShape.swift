...
/// Auto generated from SVG to SwiftUI
struct Blob: Shape {
...
struct BlobShape: View {
	...
	var body: some View {
		TimelineView(.animation) { timeline in
			...
			let firstCos = cos(angle.radians)
			let angle2 = Angle.degrees(now.remainder(dividingBy: 6) * 10)
			let secondCos = cos(angle2.radians)
			
			Canvas { context, size in
				let rectSize: CGRect = CGRect(x: 0, y: 0, width: size.width, height: size.height)
				
				context.fill(
					path(in: rectSize, x: firstCos, x2: secondCos),
					with: shadingGradient
				)
			}
		...
		.onAppear {
			withAnimation(linearShapeAnimation) {
				appear = true
			}
		}
	}
...
extension BlobShape {
	private var shadingGradient: GraphicsContext.Shading {
		.linearGradient(
			Gradient(
				colors: [.pink, .blue]),
	...
	private var linearShapeAnimation: Animation {
		.linear(duration: 20)
